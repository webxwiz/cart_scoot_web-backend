...
class AdminService {
    ...
    async getDriverWithRating(driverId, token) {
        ...
        const driverRating = await ReviewModel.aggregate()
    ...
    async getStatistic(token) {
        ...
        const users = await UserModel.aggregate().group({
            _id: '$role',
            count: {
                $sum: 1,
            },
        ...
        const totalRiders = users?.find((item) => item._id === userTypes.rider)?.count || 0;
        const totalDrivers = users?.find((item) => item._id === userTypes.driver)?.count || 0;
    ...
    async getAllAdvertisements(pageNumber, token) {
        ...
        const advertisements = await AdvertisementModel.aggregate().facet({
            data: [{ $sort: { createdAt: -1 } }, { $limit: itemsOnPage * validatePageNumber }],
        ...
        return {
            advertisements: advertisements[0]?.data,
            totalCount: advertisements[0]?.totalCount[0]?.count,
        };
    }
    ...
    async deleteAdvertisement(id, token) {
        ...
        const advertisementStatus = await AdvertisementModel.deleteOne({
            _id: id,
        });
    ...
    async changeUserRole({ id, role }, token) {
        ...
        } else if (user.role === userTypes.subadmin && role !== userTypes.admin) {
            const updatedUser = await UserModel.findOneAndUpdate(
                { _id: id },
                {
                    $set: { role },
    ...
    async sendBannedInfo(_id, banned) {
        ...
        if (user.phone?.number) {
            return await smsSender(
                banned ? `Your account has been banned by administrator` : `Your account is active`,
                user.phone.number
            );
        } else {
            await mailSender({
    ...
    async changeBunStatus(id, banned, token) {
        ...
        } else if (user.role === userTypes.subadmin) {
            const updatedUser = await UserModel.findOneAndUpdate(
                { _id: id, $or: [{ role: userTypes.rider }, { role: userTypes.driver }] },
                {
                    $set: { banned },
                },
                { new: true }
    ...
    async getAllRequests(
        { pageNumber, itemsOnPage, searchRequestCode, dateFrom, dateTo, status },
        token
    ) {
        await findAdminByToken(token);
    ...
    async getWaitingLicenses(token) {
        ...
        const users = await UserModel.find({
            'license.status': licenseTypes.waiting,
        }).sort({ createdAt: -1 });
